...
disciplineRoutes.patch("/update", [verifyToken], (req: any, res: Response) => {
  ...
  const discipline = <IDiscipline>{ modified: new Date() };
  ...
  Discipline.findByIdAndUpdate(
    ...
    (err, disciplineDB) => {
      ...
      if (!disciplineDB)
        return res.json({
          ok: false,
          desc: "There is no discipline with that ID",
        });

      return res.json({
        ok: true,
        desc: "Discipline updated",
        discipline: disciplineDB,
      });
    }
  ).catch((err) => Methods.sendErr(res, err));
...
disciplineRoutes.get("/:disciplineid", async (req: any, res: Response) => {
  ...
  const disciplines = await Discipline.findById(id)
...
disciplineRoutes.delete(
  ...
  async (req: any, res: Response) => {
    const id = req.params.disciplineid;
    await Discipline.findByIdAndDelete(id).catch((err) =>
